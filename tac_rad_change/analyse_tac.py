...
for file in files:
    ...
    if filespec[1] == 'fix': # 4CO2 fixed tsurf
        ...
        diff_ctl=my.UM.read_data('4co2_fix_tsurf')    
    elif filespec[1] == 'sw-': # 4CO2 SW-
        out_folder='/g/data/w40/dm5220/data/figures/sw-_tac_rad'
        change,area,atm=filespec[2:]
        diff_ctl=my.UM.read_data('4co2_sw_x0.9452_offset')
    else:
        raise Exception('Current script only works for 4co2_fix_tsurf or 4co2_sw- experiments.')

    out_folder_diff=out_folder+'_difference'
    ...
    diff_anom=lambda var: d[var]-diff_ctl[var]
    
    def plot_level(var,mean,func=lambda x: x*1,**kwargs):
        data=func(anom(var))
        std=func(base_anom(var)).std('time')
        P=func(d[var]).t_student_probability(func(ctl[var]),num_years=nyears) * data.std('time') / std
    ...
    def plot_level_diff(var,mean,func=lambda x: x*1,**kwargs):
        data=func(diff_anom(var))
        std=func(base_anom(var)).std('time')
        P=func(d[var]).t_student_probability(func(diff_ctl[var]),num_years=nyears) * data.std('time') / std
        data=data.annual_mean(12*nyears,normalize=std)
        if 'pressure' in data.dims:
            levs=plev
        else:
    ...
    def plot_diff(var,func=lambda x: x*1,**kwargs):
        rec=rectangle(lon=lon[area],lat=lat[area])
        if 'outpath' in kwargs:
            outpath=kwargs.pop('outpath')
            save=True
    ...
    def plot_profile(var,func=lambda x: x*1,**kwargs):
        ...
        std=sel_lat(sel_lon(func(base_anom(var)).std('time')))
        if 'pressure' in data.dims:
            levs=plev
        else:
            levs=mlev
    ...
    def plot_profile_diff(var,func=lambda x: x*1,**kwargs):
        if 'outpath' in kwargs:
            outpath=kwargs.pop('outpath')
            save=True
        else:
    ...
    plot_diff(var,
        ...
        cmap=my.Colormaps.div_tsurf,
        title=f'Surface Temperature | {changespec}',
        outpath=os.path.join(out_folder_diff,f'{changespec}_tsurf_D'))
    plt.clf()
    # PRECIP
    ...
    plot_diff(var,
        ...
        cmap=my.Colormaps.div_precip,
        title=f'Precipitation | {changespec}',
        outpath=os.path.join(out_folder_diff,f'{changespec}_precip_D'))
    plt.clf()
    # TAIR 
    # plev (lonmean)
    ...
    plot_level_diff(var,'lonmean',
        levels=plt_levels,
        du=plt_du,
        units='$K/K$',
        cmap=my.Colormaps.div_tsurf,
        title=f'Air Temperature | {changespec}',
        outpath=os.path.join(out_folder_diff,f'{changespec}_tair_lonmean_plev_D'))
    plt.clf()
    # plev (latmean)
    ...
    plot_level_diff(var,'latmean',
        levels=plt_levels,
        du=plt_du,
        units='$K/K$',
        cmap=my.Colormaps.div_tsurf,
        title=f'Air Temperature | {changespec}',
        outpath=os.path.join(out_folder_diff,f'{changespec}_tair_latmean_plev_D'))
    plt.clf()
    # mlev (lonmean)
    ...
    plot_level_diff(var,'lonmean',
        ...
        cmap=my.Colormaps.div_tsurf,
        title=f'Air Temperature | {changespec}',
        outpath=os.path.join(out_folder_diff,f'{changespec}_tair_lonmean_mlev_D'))
    plt.clf()
    # mlev (latmean)
    plot_level(var,'latmean',
        levels=plt_levels,
    ...
    plot_level_diff(var,'latmean',
        ...
        cmap=my.Colormaps.div_tsurf,
        title=f'Air Temperature | {changespec}',
        outpath=os.path.join(out_folder_diff,f'{changespec}_tair_latmean_mlev_D'))
    plt.clf()
    ...
    plot_level_diff(var,'lonmean',func=lambda x: x.to_K_per_day(),
        levels=plt_levels,
        du=plt_du,
        units='$(K \cdot d^{-1})/(K \cdot d^{-1})$',
        cmap=my.Colormaps.div_tsurf,
        title=f'LW Heating Rate | {changespec}',
        outpath=os.path.join(out_folder_diff,f'{changespec}_LWhrate_lonmean_plev_D'))
    plt.clf()
    # plev (latmean)
    ...
    plot_level_diff(var,'latmean',func=lambda x: x.to_K_per_day(),
        levels=plt_levels,
        du=plt_du,
        units='$(K \cdot d^{-1})/(K \cdot d^{-1})$',
        cmap=my.Colormaps.div_tsurf,
        title=f'LW Heating Rate | {changespec}',
        outpath=os.path.join(out_folder_diff,f'{changespec}_LWhrate_latmean_plev_D'))
    plt.clf()
    # mlev (lonmean)
    ...
    plot_level_diff(var,'lonmean',func=lambda x: x.to_K_per_day(force=True),
        levels=plt_levels,
        du=plt_du,
        units='$(K \cdot d^{-1})/(K \cdot d^{-1})$',
        cmap=my.Colormaps.div_tsurf,
        title=f'LW Heating Rate | {changespec}',
        outpath=os.path.join(out_folder_diff,f'{changespec}_LWhrate_lonmean_mlev_D'))
    plt.clf()
    # plev (latmean)
    ...
    plot_level_diff(var,'latmean',func=lambda x: x.to_K_per_day(force=True),
        levels=plt_levels,
        du=plt_du,
        units='$(K \cdot d^{-1})/(K \cdot d^{-1})$',
        cmap=my.Colormaps.div_tsurf,
        title=f'LW Heating Rate | {changespec}',
        outpath=os.path.join(out_folder_diff,f'{changespec}_LWhrate_latmean_mlev_D'))
    plt.clf()
    # SW HEATIING RATES
    # plev (lonmean)
    ...
    plot_level_diff(var,'lonmean',func=lambda x: x.to_K_per_day(),
        ...
        cmap=my.Colormaps.div_tsurf,
        title=f'SW Heating Rate | {changespec}',
        outpath=os.path.join(out_folder_diff,f'{changespec}_SWhrate_lonmean_plev_D'))
    plt.clf()
    # plev (latmean)
    plot_level(var,'latmean',func=lambda x: x.to_K_per_day(),
        levels=plt_levels,
    ...
    plot_level_diff(var,'latmean',func=lambda x: x.to_K_per_day(),
        ...
        cmap=my.Colormaps.div_tsurf,
        title=f'SW Heating Rate | {changespec}',
        outpath=os.path.join(out_folder_diff,f'{changespec}_SWhrate_latmean_plev_D'))
    plt.clf()
    # mlev (lonmean)
    ...
    plot_level_diff(var,'lonmean',func=lambda x: x.to_K_per_day(force=True),
        ...
        cmap=my.Colormaps.div_tsurf,
        title=f'SW Heating Rate | {changespec}',
        outpath=os.path.join(out_folder_diff,f'{changespec}_SWhrate_lonmean_mlev_D'))
    plt.clf()
    # plev (latmean)
    plot_level(var,'latmean',func=lambda x: x.to_K_per_day(force=True),
        levels=plt_levels,
    ...
    plot_level_diff(var,'latmean',func=lambda x: x.to_K_per_day(force=True),
        ...
        cmap=my.Colormaps.div_tsurf,
        title=f'SW Heating Rate | {changespec}',
        outpath=os.path.join(out_folder_diff,f'{changespec}_SWhrate_latmean_mlev_D'))
    plt.clf()
    # TOT HEATIING RATES
    # plev (lonmean)
    ...
    plot_level_diff(var,'lonmean',func=lambda x: x.to_K_per_day(),
        ...
        cmap=my.Colormaps.div_tsurf,
        title=f'TOT Heating Rate | {changespec}',
        outpath=os.path.join(out_folder_diff,f'{changespec}_TOThrate_lonmean_plev_D'))
    plt.clf()
    # plev (latmean)
    plot_level(var,'latmean',func=lambda x: x.to_K_per_day(),
        levels=plt_levels,
    ...
    plot_level_diff(var,'latmean',func=lambda x: x.to_K_per_day(),
        ...
        cmap=my.Colormaps.div_tsurf,
        title=f'TOT Heating Rate | {changespec}',
        outpath=os.path.join(out_folder_diff,f'{changespec}_TOThrate_latmean_plev_D'))
    plt.clf()
    # mlev (lonmean)
    ...
    plot_level_diff(var,'lonmean',func=lambda x: x.to_K_per_day(force=True),
        ...
        cmap=my.Colormaps.div_tsurf,
        title=f'TOT Heating Rate | {changespec}',
        outpath=os.path.join(out_folder_diff,f'{changespec}_TOThrate_lonmean_mlev_D'))
    plt.clf()
    # plev (latmean)
    plot_level(var,'latmean',func=lambda x: x.to_K_per_day(force=True),
        levels=plt_levels,
    ...
    plot_level_diff(var,'latmean',func=lambda x: x.to_K_per_day(force=True),
        ...
        cmap=my.Colormaps.div_tsurf,
        title=f'TOT Heating Rate | {changespec}',
        outpath=os.path.join(out_folder_diff,f'{changespec}_TOThrate_latmean_mlev_D'))
    plt.clf()
    ...
    plot_profile_diff(var,
                xlim=[-5,3],
                dx=1,
                units='$K/K$',
                title=f'Air Temperature | {changespec}',
                outpath=os.path.join(out_folder_diff,f'{changespec}_tair_vertprof_plev_D'))
    plt.clf()
    # mlev
    var='air_temperature_0'
    plot_profile(var,
    ...
    plot_profile_diff(var,
                ...
                units='$K/K$',
                title=f'Air Temperature | {changespec}',
                outpath=os.path.join(out_folder_diff,f'{changespec}_tair_vertprof_mlev_D'))
    plt.clf()
    ...
    plot_profile_diff(var,
                ...
                units='$(K \cdot d^{-1})/(K \cdot d^{-1})$',
                title=f'TOT Heating Rate | {changespec}',
                outpath=os.path.join(out_folder_diff,f'{changespec}_TOThrate_vertprof_plev_D'))
    plt.clf()
    # mlev
    var='tendency_of_air_temperature_due_to_total_heating'
    plot_profile(var,
    ...
    plot_profile_diff(var,
                ...
                units='$(K \cdot d^{-1})/(K \cdot d^{-1})$',
                title=f'TOT Heating Rate | {changespec}',
                outpath=os.path.join(out_folder_diff,f'{changespec}_TOThrate_vertprof_mlev_D'))
    plt.clf()
